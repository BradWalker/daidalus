 ...
 * BUT NOT LIMITED TO, ANY WARRANTY THAT THE SUBJECT SOFTWARE WILL 
 ...
 * RESULTING FROM USE OF THE SUBJECT SOFTWARE.  FURTHER, GOVERNMENT 
 ...
 * </li>
 * 
 * <li><p>The internal unit for angles is always radians. In this way
 * we define an units for angles, including radians and degrees.  The
 * SI convention calls a radian a dimensionless derived unit, defined as m / m.
 * </p>
 * 
 * <p>
 ...
 * incorrectly, expressed in "hertz." To accommodate this type of usage, the
 ...
 * 
 * 
 * <p>Please remember the user of the Units class should never <i>need</i> to know
 ...
 * occur automatically.  There may be reasons why the Units class would, at
 * some point in the future, use a different set of coherent base units. If
 * one always uses the <code>to</code> and <code>from</code> methods, this
 * change would be transparent.</p>
 * </li>
 * 
 ...
 * 330, 1991 Edition, The International System of Units (SI).</li>
 * 
 * <li>National Institute of Standards and Technology (NIST) Special Publication
 ...
 * of Units (SI): The Modern Metric System.</li>
 * 
 * <li>1962 Standard Atmosphere.</li>
 * 
 * <li>Bureau International des Poids et Mesures, The International System of
 * Units (SI), 7th edition, 1998.</li>
 * 
 * </ul>
 * 
 * 
...
public final class Units {
	...
	/** A string to represent degrees Celsius, using the degree symbol.  This 
	 * is a convenience to help the user avoid finding the Unicode symbol for 
	 * the degree symbol
	 */
	public static final String degreeCStr = "\u00B0C";
	/** A string to represent degrees Fahrenheit, using the degree symbol.  This 
	 * is a convenience to help the user avoid finding the Unicode symbol for 
	 * the degree symbol
	 */
	public static final String degreeFStr = "\u00B0F";
	/** A string to represent degrees Rankine, using the degree symbol.  This 
	 * is a convenience to help the user avoid finding the Unicode symbol for 
	 * the degree symbol
	 */
	...
	/**
	 * The avoirdupois pound, unit of mass. This name violates the convention of using standardized
	...
	/**
	 * <p>gn is the adopted physical constant of gravity. It is given (out of
	 ...
	 * </p>
	 * 
	 * <p>This definition is also in NIST Special Publication 330, the
	...
	/** unit of velocity, defined as a nautical mile per second */
	public static final double nautical_mile_per_second = nautical_mile / second;

	/** unit of velocity, defined as a foot per second */
	...
	static {
		...
		try {
			...
			addUnit("yard", 3.0, "ft");
			addUnit("NM", NM, "m");
			addSimilarUnit("nm", "NM");  // TODO: this should be removed. Do not add nm, nm means nanometers, not nautical miles
		...
		try {
			...
			addUnit("NM/s",nautical_mile_per_second);
			addSimilarUnit("nmi/s", "NM/s");
			addSimilarUnit("nautical_mile_per_second","NM/s");
			addSimilarUnit("nmi/s", "NM/s");
		...
		try {
			...
			addUnit("kW", 1000.0, "W");
			addSimilarUnit("kilowatt", "kW");
			addUnit("kWh", getUnitPair("kW").factor*hour, "J");
			addSimilarUnit("kilowatt-hour", "kWh");
			addUnit("V", V);
		...
		while (itr.hasNext()) {
			unspecifiedUnits.addAll((itr.next()).compatible); // unspecified is compatible with everyone
		}

		unspecifiedUnits.add("unspecified");
		unitTable.put("unspecified", new UnitPair(unspecified, unspecifiedUnits)); // this is addNewUnitPair without the error checking
		addNameToUnitTable("internal", getUnitPair("unspecified")); // this is addSimilarUnit without the error checking
	}
	...
	/**
	 * Add a UnitPair to the table of known units. Perform various error checking
	 * functions.
	 * 
	 * @param name
	 *            the new unit
	 ...
	 * 
	 */
	private static synchronized boolean addNameToUnitTable(String name,
			UnitPair pair) throws UnitException {

		if (pair.factor <= 0.0) {
			throw new UnitException("The conversion factor was not positive, this is invalid");
		}
		...
		if (unitTable.containsKey(name)) {
			UnitPair oldPair = getUnitPair(name);

			if (! pair.compatible.equals(oldPair.compatible)) {
			...
			return false; // this means the identical unit was added twice.  So don't add the unit, but don't flag an error either.
		}

		unitTable.put(name, pair);
		return true;
	}
	 ...
	 * 
	 */
	private static synchronized void addNewUnitPair(String name, double factor, SortedSet<String> compatible) throws UnitException {
		compatible.add(name);
		if (addNameToUnitTable(name, new UnitPair(factor, compatible))) {
	 ...
	 * and "ft". When two units are truly identical, this method is preferred
	...
	public static void addSimilarUnit(String newName, String originalName)
		...
		addNameToUnitTable(newName, getUnitPair(originalName));
	}
	 ...
	 * </p>
	 * 
	 * <p>This method will not make the new unit compatible with any other unit.
	...
	/**
	 * <p>Create a composite unit. For example a <i>fathom</i> unit could be
	 * defined as:<br>
	 * <code> Units.addUnit("fathom", 6.0, "foot"); </code><br>
	...
	public static void addUnit(String name, double factor, String base)
		...
		/* <p>An alternate use for this method is to define a new unit that is
		 * compatible with another unit. For instance, <br>
		 * <code> Units.addUnit("knot/s", Units.knot / Units.s, "m/s^2"); </code><br>
		...
		addNewUnitPair(name, factor * basePair.factor,	basePair.compatible);
	}

	/**
	 * <p>Returns the list of units that have the same dimensionality as the given
	 * unit.
	 * </p>
	 * 
	 * @param unit a unit
	...
	/**
	 * Parse a string into a value in internal units.  The expected use of this method is
	 * the parameter <i>str</i> comes from the user and may have errors in it; the other
	 * parameters come from the programmer so, hopefully, don't have errors in them.  Thus,
	 * a pattern like <code>Units.parse(&quot;s&quot;, from_user, -1)</code> is typical.  If
	 * the parameter <i>str</i> is invalid, then this method will behave as if <code>&quot;-1 [s]&quot;</code> 
	 * was the string supplied.<p> 
	 * 
	...
	public static double parse(String defaultUnitsFrom, String str, double defaultValue) {
		...
		if (m.matches()) { 
			...
			//   ---- -----        ----       -----          ----
			//   1    valid        valid      suppliedvalue  suppliedunit
			//   2    valid        invalid    suppliedvalue  defunit
			//   3    invalid      valid      defvalue       defunit
			//   4    invalid      invalid    defvalue       defunit
			//   5    illformed               defvalue       defunit

			if (Util.is_double(m.group(1))) {
				dbl = Util.parse_double(m.group(1));
				if (Units.isUnit(unit)) {
					dbl = Units.from(unit, dbl);      // case 1
				} else {
					dbl = Units.from(defaultUnitsFrom, dbl); // case 2	
				}
			} else {
				dbl = dv; // cases 3 and 4
			}
	 ...
	 * @param defaultUnitsFrom the default units to convert the value from (if no text unit is specified) 
	 * @param unitsTo the units to convert the value to (this will often be an internal unit)
